...
def crop_center_rectangle(img, output_size, border, border_color, crop_scale=1.5):
    ...
    larger_width = int(output_size * crop_scale)
    larger_height = int(output_size * crop_scale * 0.6)  # Adjust the height scale for rectangular shape
    
    img = img.crop(
        (
            half_the_width - larger_width/2,
            half_the_height - larger_height/2,
            half_the_width + larger_width/2,
            half_the_height + larger_height/2
        )
    )

    img = img.resize((output_size - 2*border, int((output_size - 2*border) * 0.6)))

    final_img = Image.new("RGBA", (output_size, int(output_size * 0.6)), border_color)

    mask_main = Image.new("L", (output_size - 2*border, int((output_size - 2*border) * 0.6)), 0)
    draw_main = ImageDraw.Draw(mask_main)
    draw_main.rectangle((0, 0, output_size - 2*border, int((output_size - 2*border) * 0.6)), fill=255)
...
async def get_thumb(videoid: str):
    try:
        ...
        rectangle_thumbnail = rectangle_thumbnail.resize((400, int(400 * 0.6)))
